...
import { Plus, MoreHorizontal, ChevronsLeftRight, SquareArrowOutUpRight } from "lucide-react";
...
export default function Dashboard() {
  ...
  return (
    <div className="flex flex-col h-screen bg-[#3a205a] text-gray-100 font-sans selection:bg-indigo-500/30 overflow-hidden">
      {/* HEADER */}
      <header className="bg-black/40 border-b border-white/5 backdrop-blur-md shrink-0">
        <div className="flex justify-between items-center px-6 py-3">
          ...
          <button
            ...
            className="text-sm bg-rose-500 hover:bg-rose-600 transition text-white px-4 py-1.5 rounded-lg shadow-lg active:scale-95"
          >
            Logout
          </button>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* LEFT PANEL */}
        <aside className="w-[280px] shrink-0 border-r border-white/5 bg-black/20 p-6 overflow-y-auto hidden lg:block">
          <div className="mb-8">
          ...
          <div className="rounded-xl bg-indigo-500/10 border border-indigo-500/20 p-4">
            ...
            <p className="text-xs text-white/60 leading-relaxed">Press <code className="bg-black/20 px-1 rounded">Enter</code> to quickly save a card, or <code className="bg-black/20 px-1 rounded">Esc</code> to cancel.</p>
          </div>
        </aside>
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-black/5">
          <div className="px-6 py-4 flex items-center justify-between shrink-0">
            <h2 className="text-lg font-bold text-white/90">Main Workspace</h2>
          </div>
              ...
              <div className="w-[272px] shrink-0">
                {isAddingColumn ? (
                  <div className="bg-black/40 border border-white/10 rounded-xl p-3 backdrop-blur-md shadow-2xl">
                    <ColumnInput onCancel={() => setIsAddingColumn(false)} />
                  </div>
                ) : (
                  <button
